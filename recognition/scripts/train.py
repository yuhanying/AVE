...
from scripts.valid import validate
...
def train_epoch(
    ...
    for i, (visual_input, audio_input, times, target, metadata) in enumerate(train_loader):
        ...
        # print(f'visual_input:{visual_input.shape}')
        ...
        with torch.cuda.amp.autocast(enabled=args.enable_amp):
            time_feature,time_query = model(times, "time_mlp")
            # print(time_vis.shape)
            # print(time_other.shape)
            # time_encodings = model(times, "time_mlp")

            

            inputs = [visual_input, audio_input, time_feature,time_query]
            ...
            time_encodings =[ time_feature,time_query]
        ...
        score_v = 0.0
        score_a = 0.0
        B_total = action_preds.size(0)
        # print(f'output[0][2].shape:{output[0][2].shape}')
        # print(f'output[0][3].shape:{output[0][3].shape}')
        # print(f'visual_target[:,2].shape:{visual_target[:,2].shape}')
        # print(f'audio_target.shape:{audio_target.shape}')
        for j in range(B_total):
            score_v += softmax_fn(action_preds)[j, visual_target[:,2][j]]
            score_a += softmax_fn(audio_preds)[j, audio_target[j]]
        
        ratio_v = score_v / (score_a + 1e-8)
        ratio_a = 1.0 / (ratio_v + 1e-8)
        
        alpha = 0.5
        
        if ratio_v > 1:
            coeff_v = 1 - torch.tanh(alpha * torch.relu(ratio_v))
            coeff_a = 1.0
        else:
            coeff_a = 1 - torch.tanh(alpha * torch.relu(ratio_a))
            coeff_v = 1.0
        ...
        for name, param in model.named_parameters():
            if param.grad is None:
                continue
            if 'aud' or "a" in name:
                # grad_std = param.grad.std().item() if param.grad.numel() > 1 else 1e-8
                param.grad *= coeff_a 
            if 'vis' or "v" in name:
                # grad_std = param.grad.std().item() if param.grad.numel() > 1 else 1e-8
                param.grad *= coeff_v 
