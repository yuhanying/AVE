...
class InferenceMeter(object):
    ...
    def update_epoch(self, epoch,model,args):
        if "visual" in self.modality:
            valid_indices = (self.v_labels[:, 2] != -1)
            seen = self.seen_count[valid_indices].unsqueeze(1)
        ...
        #     model_name = 'best_model.pth'  # 這個檔案會覆蓋之前相同名稱的模型
        #     saved_dict = {
        #         'saved_epoch': epoch,
        #         'model': model.state_dict(),  # 儲存模型參數
        #         'best_acc': self.best_vis_acc1,
        #     }
        #     save_dir = os.path.join(args.ckpt_path, model_name)
        #     torch.save(saved_dict, save_dir)
        #     print('The best model has been saved at {}.'.format(save_dir))
        #     print("Best acc: {:.3f}".format(self.best_vis_acc1))
        # else:
        #     # 如果這個 epoch 沒有提升，計算沒有進展的 epoch 數並印出
        #     no_improve_epochs = epoch - self.last_best_epoch
        #     print("no improve epoch: {}".format(no_improve_epochs))
        #     print("Best acc: {:.3f}".format(self.best_vis_acc1))
        ...
        if is_best_audio:
            ...
            #     'saved_epoch': epoch,
            #     'model': model.state_dict(),  # 儲存模型參數
            #     'best_acc': self.best_aud_acc1,
            # }
            # save_dir = os.path.join(args.ckpt_path, model_name)
            # torch.save(saved_dict, save_dir)
            # print('The best model has been saved at {}.'.format(save_dir))
            # print("Best acc: {:.3f}".format(self.best_aud_acc1))
        # else:
        #     # 如果這個 epoch 沒有提升，計算沒有進展的 epoch 數並印出
        #     no_improve_epochs = epoch - self.last_best_epoch
        #     print("no improve epoch: {}".format(no_improve_epochs))
        ...
        # if self.losses.avg < self.best_loss:
        #     self.best_loss=self.losses.avg
        #     self.last_best_epoch = epoch
            #  儲存模型之前確保路徑存在
            ...
            saved_dict = {
                'saved_epoch': epoch,
                'model': model.state_dict(),  # 儲存模型參數
                'best_acc': self.best_combined_acc1,
            }
            save_dir = os.path.join(args.ckpt_path, model_name)
            torch.save(saved_dict, save_dir)
            print('The best model has been saved at {}.'.format(save_dir))
            print("Best acc: {:.3f}".format(self.best_combined_acc1))
        else:
            # 如果這個 epoch 沒有提升，計算沒有進展的 epoch 數並印出
            no_improve_epochs = epoch - self.last_best_epoch
            print("no improve epoch: {}".format(no_improve_epochs))
    ...
    def update_epoch_test(self, epoch):
        if "visual" in self.modality:
            valid_indices = (self.v_labels[:, 2] != -1)
            seen = self.seen_count[valid_indices].unsqueeze(1)
