...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention, is_causal_attention=False):
        super().__init__()
        self.config = config
...
class BertSelfOutput(nn.Module):
    def __init__(self, config, is_causal_attention=False):
        super().__init__()
        if is_causal_attention:
...
class BertAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, is_causal_attention=False):
        super().__init__()
        self.self = BertSelfAttention(config, is_cross_attention, is_causal_attention)
        self.output = BertSelfOutput(config, is_causal_attention)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_causal_mask is not None:
            encoder_hidden_states_output = self.encoder_causal_attention(
                encoder_hidden_states,
                encoder_causal_mask.squeeze(1),
                head_mask,
                output_attentions=output_attentions,
                past_key_value=self_attn_past_key_value,
            )
            encoder_hidden_states = encoder_hidden_states_output[0]
